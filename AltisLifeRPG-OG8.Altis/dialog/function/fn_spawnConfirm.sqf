...
if(count playableUnits < 10) then {
    [] spawn {
        sleep 15;
        if (!(OG_WebStore_active)) then {
            [[],"TON_fnc_webStoreQuery",false,false] spawn life_fnc_MP;
        };
        sleep 3;
        diag_log format["WebStore is %1",if(OG_WebStore_active) then {"ACTIVE"} else {"INACTIVE"}];
    };
};
// adds PMC licence if they have spawned on PMC Base. That means they have the PMC licence so they can work the PMC group.
if((life_spawn_point select 0) == "pmc_spawn") then {missionNamespace setVariable ["license_pmc_basic",true]} else {missionNamespace setVariable ["license_pmc_basic",false]}