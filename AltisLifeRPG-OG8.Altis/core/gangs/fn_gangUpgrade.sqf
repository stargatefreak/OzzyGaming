...
if(_action) then {
	if(life_ogBank415 < _upgradePrice) exitWith {
		hint parseText format[
			...
			[life_ogBank415] call life_fnc_numberText,
			[_upgradePrice - life_ogBank415] call life_fnc_numberText
		];
	};
	__SUB__(life_ogBank415,_upgradePrice);
