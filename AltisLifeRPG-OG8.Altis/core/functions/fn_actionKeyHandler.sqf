...
if(isPlayer _curTarget && _curTarget isKindOf "Man") then {
	...
	if((_curTarget getVariable["restrainedCiv",false]) && !dialog && playerSide == civilian) then {
		[_curTarget] call life_fnc_civInteractionMenu;
	};
    if(side _curTarget == west && playerSide == west && !(_curTarget in (units group player)) && !dialog) then {
		[_curTarget] call life_fnc_copInteractionMenu;
    };
} else {
