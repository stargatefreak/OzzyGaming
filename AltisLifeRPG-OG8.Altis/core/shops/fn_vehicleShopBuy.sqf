...
if(life_ogCash415 < _basePrice) exitWith {hint format[localize "STR_Shop_Veh_NotEnough",[_basePrice - life_ogCash415] call life_fnc_numberText];};
...
switch(playerSide) do {
	...
	case civilian: {
		if((life_veh_shop select 2) == "civ" && {_className == "B_Heli_Light_01_F" OR _className == "C_Heli_Light_01_civil_F"}) then {
...
if(_mode) then {
	if(!(_className in ["B_G_Offroad_01_armed_F","B_MRAP_01_hmg_F"])) then {
		[[(getPlayerUID player),playerSide,_vehicle,_colorIndex,_basePrice],"TON_fnc_vehicleCreate",false,false] spawn life_fnc_MP;
	};
};
