...
_itemInfo = switch (_type) do
{
	...
	case "gold_w": {["goldore","gold_w",300,"Washing Gold",true,"water"];};
	case "gold_r": {["gold_w","gold_r",720,"Crushing Gold",false];};
	case "gold": {["gold_r","gold",1520,"Smelting Gold",false];};
...
{
	if(life_ogCash415 < _cost) exitWith {hint format[localize "STR_Process_License",[_cost] call life_fnc_numberText]; 5 cutText ["","PLAIN"]; life_is_processing = false;};
	...
	titleText[format[localize "STR_Process_Processed2",_NewVal,_itemName,[_cost] call life_fnc_numberText],"PLAIN"];
			...
			life_ogCash415 = life_ogCash415 - _cost;
