...
*/
private["_vid","_sp","_pid","_query","_sql","_vehicle","_nearVehicles","_name","_side","_tickTime","_dir","_faulty"];
...
_faulty = [_this,6,false,[false]] call BIS_fnc_param;
_unit_return = _unit;
_name = name _unit;
_side = side _unit;
_unit = owner _unit;
...
_query = format["UPDATE vehicles SET active='1', impounded='0', lastUpdated= NOW() WHERE pid='%1' AND id='%2'",_pid,_vid];
...
//_vehicle addEventHandler["Killed","_this spawn TON_fnc_vehicleDead"]; //Obsolete function?
	if(typeOf _vehicle == "C_SUV_01_F") then {_vehicle setCenterOfMass [-0.011,-0.0507,-1]};
...
if((_vInfo select 1) == "cop" && (_vInfo select 2) in ["C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F","C_Hatchback_01_sport_F","B_APC_Tracked_01_rcws_F"]) then
{
	[[_vehicle,"cop_offroad",true],"life_fnc_vehicleAnimate",_unit,false] spawn life_fnc_MP;
};

if((_vInfo select 1) == "med" && (_vInfo select 2) in ["C_Offroad_01_F","C_SUV_01_F","C_Hatchback_01_sport_F","C_Van_01_box_F","I_Truck_02_covered_F"]) then
{
