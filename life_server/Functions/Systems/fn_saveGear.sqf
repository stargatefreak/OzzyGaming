...
*/
private["_return","_uItems","_bItems","_vItems","_pItems","_hItems","_yItems","_uMags","_vMags","_bMags","_pMag","_hMag","_uni","_ves","_bag","_handled","_player"];
_player = [_this,0,objNull,[objNull]] call BIS_fnc_param;
if(isNull _player) exitWith {diag_log "SERVER SAVE GEAR ON DISCONNECT ERROR: player is objNull"};
...
if(_playerSide == west || _playerSide == civilian && {(call life_save_civ)}) then {
...
if(count (primaryWeaponMagazine _player) > 0 && alive _player) then
{
    _pMag = ((primaryWeaponMagazine _player) select 0);
    if(_pMag != "") then
    {
        _uni = _player canAddItemToUniform _pMag;
        _ves = _player canAddItemToVest _pMag;
        _bag = _player canAddItemToBackpack _pMag;
        _handled = false;
        if(_ves) then
        {
            _vMags = _vMags + [_pMag];
...
if(count (handgunMagazine _player) > 0 && alive _player) then
{
    _hMag = ((handgunMagazine _player) select 0);
    if(_hMag != "") then
    {
        _uni = _player canAddItemToUniform _hMag;
        _ves = _player canAddItemToVest _hMag;
        _bag = _player canAddItemToBackpack _hMag;
        _handled = false;
        if(_ves) then
        {
            _vMags = _vMags + [_hMag];
