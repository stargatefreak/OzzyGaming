...
if(!isServer && (!isNil "life_ozAdminlvl" OR !isNil "life_ozCoplvl" OR !isNil "life_Ozdonator")) exitWith {
	[[profileName,getPlayerUID player,"VariablesAlreadySet"],"SPY_fnc_cookieJar",false,false] spawn life_fnc_MP;
	[[profileName,format["Variables set before client initialization...\nlife_ozAdminlvl: %1\nlife_ozCoplvl: %2\nlife_Ozdonator: %3",life_ozAdminlvl,life_ozCoplvl,life_Ozdonator]],"SPY_fnc_notifyAdmins",true,false] spawn life_fnc_MP;
	sleep 0.9;
...
__CONST__(life_ozAdminlvl,parseNumber(_this select 4));
...
switch(playerSide) do {
	case west: {
		__CONST__(life_ozCoplvl, parseNumber(_this select 7));
...
if(__GETC__(life_Ozdonator) < 5) then {
	life_ozPaycheck = life_ozPaycheck + (__GETC__(life_Ozdonator) * 750);
} else {
	life_ozPaycheck = life_ozPaycheck + ((__GETC__(life_Ozdonator) * 1500) - 3000);
};
