...
/* 		if(isNil "jumpActionTime") then {jumpActionTime = 0;};
	...
	case 38: 
	{
		...
		if(_shift && playerSide in [west,independent]) then {
			if(vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F","C_Hatchback_01_sport_F","B_APC_Tracked_01_rcws_F","I_Truck_02_covered_F","C_Van_01_box_F"]) then {
	...
	case 33:
	{
		if(playerSide in [west,independent] && vehicle player != player && !life_siren_active && ((driver vehicle player) == player)) then
		{
			...
			if (_veh isKindOf "Tank") exitWith{ hint "APCs do not have sirens." };
