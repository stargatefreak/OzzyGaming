...
{
	_nearVehicles = nearestObjects[getPos _yard,["Car","Air","Ship"],30]; //Fetch vehicles within 30m.
...
if(!isNil "_deliveryTruck") then
{
	...
	if(_type == "illegal") then
	{
		...
		[[format["The truck driven by %1 has been delivered. You guys are too slow.",name player], "Anonymous Tipper",1],"clientMessage",true,false] spawn life_fnc_MP;
		[[getPlayerUID player,name player,"485"],"life_fnc_wantedListRemoveItem",false,false] spawn life_fnc_MP;
	}
	else
	{
	...
	[[_deliveryTruck],"OG_deleteVeh",false,false] spawn life_fnc_MP;
