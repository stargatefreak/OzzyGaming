...
[] spawn
{
	...
	while {true} do
	{
		...
		if(!([civilian,getPos player,100] call life_fnc_nearUnits) && (player getVariable["restrainedCiv",FALSE]) && vehicle player == player) exitWith {
...
if((player getVariable["surrender",FALSE])) then { player setVariable["surrender",FALSE,TRUE]; player switchMove ""; sleep 0.5;};
